###########################
#
# Executes the Streams Coordination Command line.
# e.g. streams.sh -start coordination
#      streams.sh -ls
#!/usr/bin/env bash
...
CLASS="org.streams.coordination.main.Coordination"

CLASSPATH="$STREAMS_CONF_DIR:$STREAMS_CONF_DIR/META-INF:$CLASSPATH"



exec "$JAVA" -Xss128k -XX:+UseConcMarkSweepGC -XX:+UseParNewGC -XX:+DisableExplicitGC $JAVA_HEAP $JAVA_OPTS -classpath "$CLASSPATH" $CLASS "$@"
