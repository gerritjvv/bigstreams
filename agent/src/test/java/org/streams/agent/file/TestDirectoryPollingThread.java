...
public class TestDirectoryPollingThread extends TestCase{
	...
	public void testSeeUpdateErrorFileSizeDecrement() throws Exception {
		// Check to see that the DirectoryPollingThread sees when a filesize is
		// smaller
		...
		assertEquals(updateFileSizeDecrementFilesArr.length,
				memory.getFileCount());
		...
		for (int i = 0; i < updateLength; i++) {
			...
			assertEquals(FileTrackingStatus.STATUS.READ_ERROR,
					status.getStatus());
		}
	...
	public void testSeeUpdatesUpdate() throws Exception {
		...
		for (int i = 0; i < updateLength; i++) {
			...
			FileUtils.writeStringToFile(file,
					"New Data this is a long string than the file has had");
		}
	...
	public void setUp() throws Exception {
		...
		updateFileSizeDecrementDir = new File(baseDir,
				"updateFileSizeDecrementDir");
		...
		for (int i = 0; i < newFileCount; i++) {
			...
			// create files to update
			File updateFileSizeDecrement = new File(updateFileSizeDecrementDir,
					"testFile_" + i + ".log");
