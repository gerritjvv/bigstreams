...
/**
 * 
 * Maintains a ConcurrentHashMap with an instance of CompressionPool for each CompressionCodec.
 */
public class CompressionPoolFactoryImpl implements CompressionPoolFactory{

	private static final Logger LOG = Logger.getLogger(CompressionPoolFactoryImpl.class);
		...
		if(pool == null){
			...
			try{
				...
				if(pool2 == null){
					...
					pool2 = new CompressionPoolImpl(codec, decompressorPoolSize, compressorPoolSize, status);
